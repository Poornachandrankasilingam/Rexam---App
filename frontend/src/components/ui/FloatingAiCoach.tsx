...
export function FloatingAiCoach() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="w-[360px] sm:w-[400px] h-[520px] rounded-3xl bg-white border border-slate-200 shadow-2xl flex flex-col overflow-hidden animate-fade-up">
          {/* Header */}
          <div className="p-4 bg-emerald-600 text-white flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h4 className="text-xs font-black tracking-tight leading-tight">Rexam AI Quick Coach</h4>
            ...
            <div className="flex items-center space-x-1">
              <Link to="/student/ai-coach" onClick={() => setIsOpen(false)}>
                <button className="text-[10px] px-2 py-1 bg-white/20 hover:bg-white/30 rounded-lg font-bold text-white transition-colors">
                  Full Room ↗
                </button>
              </Link>
              <button
          ...
          {/* Messages */}
          <div className="flex-1 p-4 overflow-y-auto space-y-3 custom-scrollbar text-xs bg-slate-50">
            {messages.map((m, idx) => (
              <div
                key={idx}
                ...
                <div
                  className={`max-w-[85%] p-3 rounded-2xl ${
                    m.role === "user"
                      ...
                      : "bg-white border border-slate-200 text-slate-900 rounded-tl-none shadow-sm space-y-1"
                  }`}
                >
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-white border border-slate-200 p-2.5 rounded-2xl text-[11px] font-bold text-slate-600 flex items-center space-x-2 shadow-sm">
          ...
          <div className="px-3 py-2 bg-emerald-50 border-t border-emerald-100 flex items-center justify-between text-[11px]">
            <span className="font-bold text-emerald-900">Want live oral viva?</span>
            <Link
              to="/student/ai-coach"
              onClick={() => setIsOpen(false)}
              className="text-emerald-700 font-extrabold flex items-center hover:underline"
            >
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-200 flex items-center space-x-2">
            <input
              ...
              className="flex-1 p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-medium text-slate-900 focus:outline-none focus:border-emerald-500"
            />
            <Button
              type="submit"
              size="sm"
              disabled={!input.trim() || loading}
              className="btn-3d-green rounded-xl h-9 px-3 text-white"
            >
